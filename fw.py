...
def asavg():
    ...
    response = post_the_json(config, "http://10.6.80.11/api/node/mo/uni/%s.json" % (device["tn-t_name"]))
    os.remove("output")
    # if response:
    ...
    json_file = "output"
    config = json_load(json_file)
    post_the_json(config, "http://10.6.80.11/api/node/mo/uni/%s.json" % (device["tn-t_name"]))
    os.remove("output")
    if response:
